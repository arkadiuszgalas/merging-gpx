...
def check_naming_convention(gpx_files_input_list):
   ...
   for filename in gpx_files_input_list:
      if filename.find("_") != -1 and filename[filename.find("_") + 1:].find("_") != -1:
         file_is_correct = True
      else:
...
if input_dir.exists():
   ...
   if len(files_list) > 1:
      ...
      if namesCorrect:
         ...
         dir_name_input = os.path.join(str(dir_name),'input')
         ...
         if gpx_valid:
            ...
            print("Validation of gpx filed. There are ovelapping ranges or duplicates in data.")
      else:
